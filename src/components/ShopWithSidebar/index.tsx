...
const ShopWithSidebar = () => {
  ...
  // Logic ItemsPerPage động
  const itemsPerPage = useMemo(() => {
    if (sortBy === "bestseller") return 6;
    if (sortBy === "newest") return 8;
    return 9;
  }, [sortBy]);
  ...
  const { data: allProductsForCountData } = useProducts();

  const allProductsForCount = useMemo(() => {
    return allProductsForCountData?.products || [];
  }, [allProductsForCountData]);

  // Tính toán Min/Max mặc định từ DB
  const priceRangeData = useMemo(() => {
    ...
    allProductsForCount.forEach((product) => {
      if (!product.hasVariants) {
        allPrices.push(product.discountedPrice ?? product.price);
      } else {
        if (product.variants && product.variants.length > 0) {
          product.variants.forEach((variant) => {
            allPrices.push(variant.discountedPrice ?? variant.price);
          });
        } else {
          allPrices.push(product.discountedPrice ?? product.price);
        }
    ...
    if (allPrices.length === 0) return { min: 0, max: 100000000 };
  ...
  const {
    ...
    // Truyền giá trị lọc vào hook - Hook sẽ tự động gọi API khi giá trị này thay đổi
  ...
  // Hàm xử lý khi nhấn Áp dụng ở PriceDropdown
  const handlePriceChange = (minPrice: number, maxPrice: number) => {
    console.log("ShopWithSidebar nhận được giá:", minPrice, maxPrice);

    // Nếu user chọn khoảng trùng với mặc định -> set null để bỏ lọc
    if (minPrice <= priceRangeData.min && maxPrice >= priceRangeData.max) {
      setPriceRange(null);
    } else {
  ...
  const categories = useMemo(() => {
    ...
    return categoriesData.map((category) => {
      const apiCount = (category as any)._count?.products;
      const manualCount =
        allProductsForCount?.filter((p) => p.categoryId === category.id)
          .length || 0;
      return {
        id: category.id,
        name: category.title,
        products: apiCount !== undefined ? apiCount : manualCount,
        isRefined: selectedCategoryId === category.id,
      };
    });
  }, [categoriesData, selectedCategoryId, allProductsForCount]);
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: any) {
      if (!event.target.closest(".sidebar-content")) setProductSidebar(false);
    }
    if (productSidebar)
          ...
          <div className="flex gap-7.5">
            ...
            >
              ...
              <div className="flex flex-col gap-6">
                ...
                {/* Price Dropdown - Truyền props đúng */}
                <PriceDropdown
                  // Nếu đang lọc thì dùng giá lọc, nếu không thì dùng giá Min/Max toàn hệ thống
                  minPrice={priceRange?.min ?? priceRangeData.min}
                  maxPrice={priceRange?.max ?? priceRangeData.max}
            ...
            <div className="xl:max-w-[870px] w-full">
              ...
              {isLoading ? (
                <div
                  className={`${productStyle === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-7.5 gap-y-9" : "flex flex-col gap-7.5"}`}
                >
                    ...
                    <ul className="flex items-center">
                      <li>
                        <button
                          ...
                          className={`flex items-center justify-center w-8 h-9 ease-out duration-200 rounded-[3px] ${currentPage === 1 ? "text-gray-4 cursor-not-allowed" : "hover:text-white hover:bg-blue"}`}
                        >
                          <svg
                            className="fill-current"
                            width="18"
                      ...
                      {getPageNumbers().map((page, index) => (
                        <li key={index}>
                          {page === "..." ? (
                            <span className="flex py-1.5 px-3.5 text-dark">
                              ...
                            </span>
                          ) : (
                            <button
                      ...
                      <li>
                        <button
                          ...
                          className={`flex items-center justify-center w-8 h-9 ease-out duration-200 rounded-[3px] ${currentPage === totalPages ? "text-gray-4 cursor-not-allowed" : "hover:text-white hover:bg-blue"}`}
                        >
                          <svg
                            className="fill-current"
                            width="18"
