...
const ShopWithSidebar = () => {
  ...
  // Fetch all products (for counting products per category)
  ...
  // Calculate min/max price from all products
  // Min is always 0, max is the highest price in database (including variants)
  const priceRangeData = useMemo(() => {
    if (!allProductsForCount || allProductsForCount.length === 0) {
      return { min: 0, max: 100000000 };
    }

    // Collect all prices including variants
    const allPrices: number[] = [];

    allProductsForCount.forEach((product) => {
      // Add product price (for products without variants)
      if (!product.hasVariants) {
        allPrices.push(product.discountedPrice ?? product.price);
      } else {
        // For products with variants, add all variant prices
        if (product.variants && product.variants.length > 0) {
          product.variants.forEach((variant) => {
            allPrices.push(variant.discountedPrice ?? variant.price);
          });
        } else {
          // Fallback to product price if no variants found
          allPrices.push(product.discountedPrice ?? product.price);
        }
      }
    });
    ...
    // Always start from 0, max is the highest price
    return { min: 0, max: Math.ceil(max) };
  }, [allProductsForCount]);

  // Fetch products from API with category and price filter and pagination
  const {
    data: productsData,
    isLoading,
    isError,
  ...
  // Map categories from API to format needed by CategoryDropdown
  ...
  // Products to display (filtered by category if selected)
  ...
  const handlePriceChange = (minPrice: number, maxPrice: number) => {
    // Only set price range if it's different from the full range
