...
const Categories = () => {
  ...
  // Don't render Swiper until mounted to prevent hydration mismatch
  if (!mounted) {
    return (
            ...
            <div className="flex gap-4">
              {Array.from({ length: 6 }).map((_, index) => (
                <div
                  ...
                  className="h-40 rounded-2xl bg-gray-2 animate-pulse flex-shrink-0 w-[130px]"
                />
              ))}
            </div>
          </div>
      ...
      <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0 pb-15 border-b border-gray-3">
        <div className="swiper categories-carousel common-carousel">
          ...
          >
            {isLoading
              ? Array.from({ length: 6 }).map((_, index) => (
                  <SwiperSlide key={`category-skeleton-${index}`}>
                    <div className="h-40 rounded-2xl bg-gray-2 animate-pulse" />
                  </SwiperSlide>
                ))
              : categories.map((item) => (
          ...
          {isError && (
            <p className="text-sm text-red-500 mt-4">
              Unable to load latest categories. Showing demo data instead.
            </p>
          )}
        </div>
      </div>
