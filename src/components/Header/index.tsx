...
// --- COMPONENT SKELETON (KHUNG XƯƠNG) ---
// Giúp giữ chỗ khi dữ liệu đang tải, tránh giật giao diện
const UserInfoSkeleton = () => (
  <div className="flex items-center justify-end gap-3 animate-pulse w-full">
    {/* Avatar Skeleton */}
...
const Header = () => {
  ...
  // Lưu ý: Nếu useAuth của bạn có trả về 'isLoading', hãy thêm vào đây
  // Ví dụ: const { isAuthenticated, user, logout, isLoading } = useAuth();
  // Ở đây mình giả định dùng 'mounted' để làm loading state ban đầu
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("scroll", handleStickyMenu); // Cleanup listener
  });
  ...
  const categoryOptions = useMemo(() => {
    const allCategoriesOption = { label: "Tất cả danh mục", value: "" };
    if (categories && categories.length > 0) {
      const categoryOptionsList = categories.map((cat) => ({
        label: cat.title,
        value: cat.id,
      }));
      return [allCategoriesOption, ...categoryOptionsList];
    }
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (trimmedQuery) {
      ...
      router.push(`/shop-without-sidebar?${params.toString()}`);
    } else if (selectedCategoryId) {
      router.push(`/shop-without-sidebar?categoryId=${selectedCategoryId}`);
    }
  };
    ...
    >
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
        ...
        >
          ...
          <div className="xl:w-auto flex-col sm:flex-row w-full flex sm:justify-between sm:items-center gap-5 sm:gap-10">
            <Link className="flex-shrink-0" href="/">
              <Image
                ...
                width={219}
                height={36}
                priority={true} // Tải ngay lập tức để không bị chớp
          ...
          {/* */}
          <div className="flex w-full lg:w-auto items-center justify-end gap-7.5">
            <div className="flex w-full lg:w-auto justify-end items-center gap-5">
              {/* === USER INFO SECTION === */}
              {/* Bao bọc trong div có min-width để tránh bị nhảy layout */}
              <div className="flex items-center gap-5 min-w-[200px] justify-end">
                {/* Nếu chưa mount xong -> Hiển thị Skeleton */}
                {!mounted ? (
                  <UserInfoSkeleton />
                  ...
                  <div className="relative user-menu-container">
                    ...
                    >
                      ...
                      {user.avatar ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-blue flex items-center justify-center text-white font-semibold text-sm">
                          {user.name
                            ? user.name
                                .split(" ")
                                .map((n) => n[0])
                      ...
                      {/* Dropdown Arrow */}
                      <svg
                        className={`w-4 h-4 text-dark-4 transition-transform duration-200 ${
                    ...
                    {userMenuOpen && (
                      <>
                        ...
                        ></div>
                        <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-3 z-20">
                          <div className="py-2">
                            <Link
                              href="/my-account"
                              onClick={() => setUserMenuOpen(false)}
                              className="block px-4 py-2 text-sm text-dark hover:bg-gray-1 transition-colors"
                            >
                              Tài khoản của tôi
                            </Link>
                            <Link
                            ...
                            {user.roles && user.roles.includes("ADMIN") && (
                              <>
                                <div className="border-t border-gray-3 my-1"></div>
                                <Link
                                  href="/admin/dashboard"
                                  onClick={() => setUserMenuOpen(false)}
                                  className="block px-4 py-2 text-sm text-blue font-medium hover:bg-blue-50 transition-colors"
                                >
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center text-custom-sm text-dark font-medium whitespace-nowrap">
                ...
                >
                  ...
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M12 2C8.13 2 5 5.13 5 9C5 14.25 2 16 2 16H22C22 16 19 14.25 19 9C19 5.13 15.87 2 12 2ZM6.5 9C6.5 5.96 8.96 3.5 12 3.5C15.04 3.5 17.5 5.96 17.5 9C17.5 13.5 19.5 14.5 19.5 14.5H4.5C4.5 14.5 6.5 13.5 6.5 9Z"
                      fill="#3C50E0"
                    />
                    <path
                      d="M9 21C9 22.1046 9.89543 23 11 23H13C14.1046 23 15 22.1046 15 21H9Z"
                      fill="#3C50E0"
                    />
                  </svg>
      ...
      {/* Phần menu dưới giữ nguyên không thay đổi */}
            ...
            <div className="hidden xl:block">
              <ul className="flex items-center gap-5.5">
                ...
                <li className="py-4">
                  <Link
                    href="/wishlist"
                    className="flex items-center gap-1.5 font-medium text-custom-sm text-dark hover:text-blue relative"
                  >
                    <svg
                      className="fill-current"
                      width="16"
                    ...
                    {mounted &&
                      ...
                      wishlistItems.length > 0 && (
                        <span className="flex items-center justify-center font-medium text-2xs bg-blue w-4.5 h-4.5 rounded-full text-white ml-0.5">
