...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const { data: ratingData, isLoading: ratingLoading } = useProductRating(
    item.id
  );
  ...
  const handleAddToCart = () => {
    // Nếu sản phẩm có variants, phải chọn variant trước (redirect đến trang chi tiết)
    if (item.hasVariants) {
      toast.error(
        "Vui lòng chọn các tùy chọn sản phẩm trước khi thêm vào giỏ hàng"
      );
      router.push(`/shop-details?id=${item.id}`);
      return;
    }

    // Sản phẩm không có variants, có thể thêm trực tiếp
    addItemToCart({
      id: item.id,
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        <Image src={primaryImage} alt={item.title} width={250} height={250} />
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <WishlistButton productId={item.id} product={item} />
        </div>
      </div>

      {!ratingLoading && totalReviews > 0 ? (
        <StarRating
        ...
        <p className="text-custom-sm text-gray-5 mb-2">Chưa có đánh giá</p>
      ) : null}
      ...
      >
        <Link href={`/shop-details${item.id ? `?id=${item.id}` : ""}`}>
          {item.title}
        </Link>
      </h3>
        ...
        {item.hasVariants ? (
          <>
            <span className="text-dark">
              Từ {formatPrice(item.discountedPrice ?? item.price)}
            </span>
          </>
        ) : (
          <>
