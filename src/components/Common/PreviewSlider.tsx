...
const PreviewSliderModal = () => {
  ...
  const product = useProductDetailsStore((state) => state.value);

  const sliderRef = useRef(null);

  // Get images from product, fallback to empty array
  ...
  if (!isModalPreviewOpen || images.length === 0) {
    return null;
  }

  return (
    <div className="preview-slider w-full h-screen z-999999 inset-0 fixed flex justify-center items-center bg-[#000000F2] bg-opacity-70">
      <button
      ...
      <div>
        <button
          className="rotate-180 absolute left-4 sm:left-8 lg:left-16 p-5 cursor-pointer z-10"
          onClick={handlePrev}
        ...
        <button
          className="absolute right-4 sm:right-8 lg:right-16 p-5 cursor-pointer z-10"
          onClick={handleNext}
